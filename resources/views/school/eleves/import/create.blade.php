...
@section('content')
    ...
    <div x-data="studentImport()" class="settings-grid">
        ...
        <div x-show="toast.show" x-transition class="toast"
            :style="toast.type === 'error' ? 'background:#dc2626' : 'background:#16a34a'" x-text="toast.message"></div>
        ...
        <div class="card section-spacing" style="background:#f9fafb; border-left:5px solid #2563eb;">
            ...
            <a href="{{ route('modele.eleve.download') }}" class="import-btn"
                style="background: #ffffff; border: 1px solid #d1d5db; color: #374151; text-decoration: none;">
                    ...
                    <div style="flex:1; min-width:220px;">
                        <select x-model="classe_id" @change="checkSerie()" class="form-input">
                            ...
                            @foreach ($classes as $c)
                                <option value="{{ $c->id }}" data-nom="{{ $c->nom }}">{{ $c->nom }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <tbody>
                        <template x-for="(s,i) in students" :key="i">
                            <tr>
                                ...
                                <td
                                    style="white-space:nowrap; text-align: center; display: flex; gap: 5px; justify-content: center">
                                    <button type="button" class="circle-btn btn-add" title="Ajouter après"
                                        @click="addAfter(i)">
                                    ...
                                    <button type="button" class="circle-btn btn-delete" title="Supprimer"
                                        @click="remove(i)">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                ...
                <div class="modal-content">
                    ...
                    <div style="display: flex; flex-direction: column; gap: 10px;">
                        <template x-if="statusModal.showCanvasBtn">
                            <a href="{{ route('modele.eleve.download') }}" class="import-btn"
                                style="background: #f3f4f6; color: #1f2937; text-decoration: none;">
        ...
        function studentImport() {
            return {
                ...
                checkSerie() {
                    ...
                    this.showSerie = (nom.includes('2nde') || nom.includes('seconde') || nom.includes('1ère') || nom
                        .includes('tle'));
