...
@section('content')
    ...
    <div x-data="studentImport()" class="settings-grid">
        ...
        <!-- TOAST -->
        <div x-show="toast.show" x-transition class="toast" :class="toast.type" x-text="toast.message">
        </div>
                    ...
                    <tbody>
                        <template x-for="(s,i) in students" :key="i">
                            <tr>
                                ...
                                <td>
                                    <input type="date" class="form-input" x-model="s.date_naissance">
                                </td>

                                <td>
                                    <input class="form-input" x-model="s.lieu_naissance">
                                </td>

                                <td>
                                    <input class="form-input" x-model="s.telephone_tuteur">
                                </td>

                                <td style="white-space:nowrap; text-align:center;">
                                    <button type="button" class="circle-btn btn-add" @click="addAfter(i)">
                                        +
                                    </button>

                                    <button type="button" class="circle-btn btn-delete" @click="remove(i)">
                                        🗑
                                    </button>
                                </td>
                            </tr>
    ...
    <script>
        function studentImport() {
            return {
                ...
                // async upload() {
                //     if (!this.file) {
                //         this.notify('Choisir un fichier', 'error');
                //         return;
                //     }
                ...
                async upload() {
                    ...
                    try {
                        ...
                        for (let i = 3; i < rows.length; i++) {
                            ...
                            const rowNumber = i + 1; // numéro réel Excel
                            const row = rows[i];

                            this.students.push({
                                photo: images[rowNumber] ? images[rowNumber].image : null,
                                matricule: row[2] ?? '',
                                nom: row[3] ?? '',
                                prenom: row[4] ?? '',
                                sexe: row[5] ?? '',
                                date_naissance: formatDate(row[7]),
                                lieu_naissance: row[8] ?? '',
                                telephone_tuteur: row[9] ?? ''
                            });
                        }
                ...
                async saveAll() {
                    if (!this.classe_id) {
                        this.notify('Choisir une classe', 'error');
                        return;
                    }
                    if (this.showSerie && !this.serie) {
                        this.notify('Choisir la série', 'error');
                        return;
                    }
                    if (!this.validateStudents()) return;

                    try {
                        const response = await fetch("{{ route('admin.students.import.storeAll') }}", {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
